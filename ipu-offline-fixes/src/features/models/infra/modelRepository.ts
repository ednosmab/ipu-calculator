...
export const modelRepository = {
  ...
  async create(model: CalculationModel): Promise<boolean> {
    return withWriteLock(async () => {
      ...
      // Bug #2 Fix: quando offline, registra na fila de pendentes para que
      // processPendingEditsUseCase possa fazer retry com controle de tentativas.
      // Antes, o modelo ficava como 'pending' no storage mas nunca entrava
      // na fila, sendo ignorado pelo mecanismo de retry.
      if (!isSynced) {
